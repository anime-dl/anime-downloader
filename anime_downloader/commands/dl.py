...
@click.option(
    '--download-number','-dn',type=int,
    help='Given Anime number starts downloading. '
)
@click.pass_context
def command(ctx, anime_url, episode_range, url, player, skip_download, quality,
            force_download, download_dir, file_format, provider,
            external_downloader, chunk_size, disable_ssl, fallback_qualities,download_number):
    ...
    if not cls:
        anime_url = util.search(anime_url, provider,download_number)
        cls = get_anime_class(anime_url)
