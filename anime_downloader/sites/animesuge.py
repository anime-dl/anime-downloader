...
class AnimeSuge(Anime, sitename="animesuge"):
    ...
    def _scrape_episodes(self):
        ...
        _id=re.search(".+-(.*)", self.url).group(1)
...
class AnimeSugeEpisode(AnimeEpisode, sitename='animesuge'):
    def _get_sources(self):
        ...
        _id, ep_no=re.search(".*\/anime\/.*?-(.*?)\/.*-(\d+)$", self.url).group(1,2)
        ...
        data_sources=json.loads(helpers.soupify(helpers.get("https://animesuge.io/ajax/anime/servers", params={"id":_id, "episode": ep_no})).select(f"a[data-base='{ep_no}']")[0].get("data-sources"))
        ...
        for key in id_source_map:
            if key in data_sources.keys():
                ...
                link=helpers.get("https://animesuge.io/ajax/anime/episode", params={"id": _id}).json()['url']
